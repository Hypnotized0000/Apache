...
/*
 * SecureChannel — the live demo. The browser talks directly to the Arduino over
 * USB serial (Web Serial API), so there is no server in between. Typed text is
 * encrypted here, sent as raw bytes to the device, decrypted and shown on its
 * screen; the device replies encrypted and we decrypt it back here.
 *
 * Web Serial only exists in Chromium browsers (Chrome / Edge) on desktop, which
 * is what the booth Mac runs. Everything degrades to a clear notice elsewhere.
 */

const BAUD_RATE = 115200;
const MAX_MESSAGES = 60;

// Canned messages so a passer-by can fire off something without typing.
...
// Render the bytes that actually travel over the cable, grouped for readability.
...
// A line is an encrypted payload only if it is pure hex with a nonce + body.
